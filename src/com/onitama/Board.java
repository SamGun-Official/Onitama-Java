...
public class Board {
    ...
    public void switchPlayer() {
        ...
        // System.out.println(temp.isComputer());
            ...
            for (Card c : node.getCardsOfPlayerColor(aiCurPlayer.getColor())) {
                for (Piece p : node.getPiecesOfColor(aiCurPlayer.getColor())) {
                    ...
                    for (int[] move : c.getMoves()) {
                        ...
                        if (!to.move(move, aiCurPlayer.getColor())) {
                            continue;
                        }
                        if (child.getPiece(from) == null) {
                            continue;
                        }
                        if (child.getPiece(to) != null && child.getPiece(from).getColor() == child.getPiece(to).getColor()) {
                            continue;
                        }
            ...
            for (Card c : node.getCardsOfPlayerColor(aiOffPlayer.getColor())) {
                ...
                for (Piece p : node.getPiecesOfColor(aiOffPlayer.getColor())) {
                    ...
                    for (int[] move : c.getMoves()) {
                        ...
                        if (!to.move(move, aiOffPlayer.getColor()) || from.equals(to)) {
                            continue;
                        }
                        if (child.getPiece(from) == null) {
                            continue;
    ...
    public void printBoard(Piece[][] child) {
        for (int i = 0; i < 5; i++) {
            for (int j = 0; j < 5; j++) {
                System.out.printf("|%2s ", (child[i][j] == null) ? "" : (child[i][j].getColor() == red) ? "r" : "b");
            }
            System.out.println("|");
        }
    }
